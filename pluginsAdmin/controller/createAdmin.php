...
if(isset($_POST['identifiant']) AND isset($_POST['password']) AND isset($_POST['email']) AND isset($_POST['confirmation']) )
{
	...
	if($message != "")
	{
		...
		header("Location: ../view/creationCompteAdmin.php");
	}
	else
	{
		...
		$utilisateur = new Utilisateur($login, $pass);

		if($utilisateur->ifAdminExist())
		{
			$_SESSION["alert_creation"] = "<p>Ce pseudo est déjà utilisé !!</p>";
			header("Location: ../view/creationCompteAdmin.php");
		}
		else
		{
			
			$utilisateur->createAdmin($login, $pass, $mail);
